...
def run_backtest(df, short_window, long_window, initial_capital, use_macd_filter=False):
    ...
    if not use_macd_filter:
        # === 原始策略：純均線交叉 ===
        # 黃金交叉即買入 (Signal=1)，死亡交叉即賣出 (Signal=0)
        mask = data['Short_MA'] > data['Long_MA']
        data.loc[mask, 'Signal'] = 1
    else:
        # === 進階策略：均線 + MACD 濾網 ===
        # 買入：均線黃金交叉 (短>長) AND MACD > 0
        # 賣出：均線死亡交叉 (短<長) AND MACD < 0
        # 其他情況：維持現狀 (Hold)
    ...
    if stock_id:
        ...
        if df is not None and not df.empty:
            ...
            if run_backtest_btn:
                ...
                fig_bt = go.Figure()
                fig_bt.add_trace(go.Scatter(x=res1.index, y=res1['Total_Asset'], mode='lines', name=f'策略 A (純MA)', line=dict(color='gold', width=2)))
                fig_bt.add_trace(go.Scatter(x=res2.index, y=res2['Total_Asset'], mode='lines', name=f'策略 B (MA+MACD)', line=dict(color='cyan', width=2, dash='dot')))
