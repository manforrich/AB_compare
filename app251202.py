...
if app_mode == "📊 單一個股分析":
    ...
    show_macd = st.sidebar.checkbox("MACD", True) # 新增 MACD 選項
    ...
    if stock_id:
        ...
        if df is not None and not df.empty:
            ...
            # 2. 繪圖 (調整為 3 列)
            # row_heights 分配: K線 60%, 成交量 15%, MACD 25%
            fig = make_subplots(
                rows=3, cols=1, 
                shared_xaxes=True, 
            ...
            if run_backtest_btn:
                ...
                # 接收 回傳數據 與 交易紀錄
                res1, log1 = run_backtest(df, s1_short, s1_long, initial_capital)
                res2, log2 = run_backtest(df, s2_short, s2_long, initial_capital)
